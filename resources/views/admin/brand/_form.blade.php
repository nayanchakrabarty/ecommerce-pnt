    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                @if(isset($brand) && $brand->logo != null)
                    <img src="{{ asset($brand->logo) }}" alt="">
                @endif
                <input name="logo" value="{{ old('logo',isset($brand)?$brand->logo:null) }}" type="file" id="logo" class="form-control form-control-line @error('logo') is-invalid @enderror">
            </div>
            @error('logo')
            <div class="pl-1 text-danger">{{ $message }}</div>
            @enderror
